---
name: spider-mind
description: 三阶渐进式爬虫逆向框架。从接收目标 URL 开始，先裸 Python 探路，受阻后启用 Playwright 配合补环境扣代码，最后以 jsreverser 深度逆向兜底，交付从零到一的完整爬虫项目脚手架。适用场景：用户提供目标页面 URL、API 地址、JS 片段、加密参数样本，或要求搭建完整爬虫采集项目。
---
...
- 拿到目标后，第一件事永远是裸 Python 直连探路，不跳过直接上浏览器。
- Playwright 只用于调试观察和定位加密点，不是最终交付方案。
...
1. **目标指纹**
   - 确认目标 URL 是否可访问
   - 判断是公开数据还是需要登录
   - 初步识别反爬类型：IP 封禁、UA 检测、验证码、JS 挑战、加密参数、WASM

2. **阶位决策**
   - 先发一个裸 Python GET/POST 看响应
   - 根据结果决定进入第几阶（见 `references/strategy-triage.md`）
...
- `references/workflow-overview.md` 最短端到端执行地图
- `references/strategy-triage.md` 拿到目标后选哪一阶的决策树
- `references/stage-1-pure-py.md` 裸 Py 探路 playbook
- `references/stage-2-env-patch.md` 补环境扣代码 playbook
- `references/stage-3-jsreverser.md` 深度逆向 playbook
- `references/escalation-ladder.md` 升级阶梯，防止跳级
- `references/code-quality.md` 代码质量检查清单
- `references/dev-doc.md` 开发文档规范与模板
- `references/troubleshooting.md` 故障排查和环境问题
- `references/report-templates.md` 阶段报告和交付报告模板
...
入口和决策：
- `references/workflow-overview.md` 完整流程地图（从阶段 0 到阶段 6）
- `references/strategy-triage.md` 目标到手 → 阶位选择
...
1. 拿到目标 → 先发请求看结果 → 再决定用多重的武器
2. 代码写好 → 加上中文注释 → 质量检查 → 写开发文档
